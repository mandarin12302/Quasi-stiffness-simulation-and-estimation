...
numbers = 1;
subject = 2;
fp = fopen('GRF_sub2_p.txt','at');%GRF_nagative py.txt?initial
% name
fprintf(fp,'%s\r',name);
% rows and columns
fprintf(fp,'%s',nRows);
...
GRF_px = zero;
GRF_py = KINETICS_data.Mean_COP{1,1}.COP_Data{1,1};
GRF_pz = zero;
%GRF_pz = -0.932*one;

GRF_vy = KINETICS_data.Mean_GRF{1,1}.GRF_Data{1,1}.*mass.*g;
GRF_vz = KINETICS_data.Mean_GRF{1,1}.GRF_Data{2,1}.*mass.*g;
...
for k = 1:rows
    ...
    fprintf(fp,'%8.8f\t',GRF_vy(subject,k));%2 opensim vx
    fprintf(fp,'%8.8f\t',GRF_vz(subject,k));%3 vy
    fprintf(fp,'%8.8f\t',zero(k));%4 vz 0
    fprintf(fp,'%8.8f\t',GRF_py(subject,k));%5 px 
    fprintf(fp,'%8.8f\t',zero(k));%6 opensim pyGRF_pz(subject,k)
    fprintf(fp,'%8.8f\t',zero(k));%7 pz 0 GRF_px(subject,k)
    ...
    fprintf(fp,'%8.8f\t',zero(k));%12 pyGRF_pz(subject,k)
    fprintf(fp,'%8.8f\t',zero(k));%13 pz 0GRF_px(subject,k)
